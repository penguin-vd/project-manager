    ...
    while (true) {
        ...
        for (size_t i = 0; i < projects.size(); i++) {
            ...
            if (has_todo && is_selected) {
                ...
                insert_colored(buffer, style_buffer, 1, i + 4, projects[i].name, "\033[91m");
            } else if (is_selected) {
                insert_colored(buffer, style_buffer, 1, i + 4, projects[i].name, "\033[7;3m");
        ...
        if (ch == 27) {
            ...
            for (size_t i = 0; i < projects.size(); i++) {
                std::string info = "Fetching: (" + std::to_string(i) + "/" + std::to_string(projects.size()) + ")";
...
void project_menu(sqlite3 *db, std::vector<std::string> &buffer, int screen_width,
                  int screen_height, project &project) {
    ...
    while (true) {
        ...
        // LEFT BUFFER
        insert_colored(left_buffer, left_style_buffer, 1, 1, "Project Tree", x == 0 ? "\033[7;1m" : "\033[1m");
        ...
        if (left_info.length() < left_width - 1) {
            insert_colored(left_buffer, left_style_buffer, 1, 2, left_info, "\033[3;36m");
        } else {
            size_t pos = 0;
            while (left_info.size() > pos) {
                std::string info = left_info.substr(pos, left_width - 1);
                insert_colored(left_buffer, left_style_buffer, 1, 2 + info_offset, info, "\033[3;36m");
                pos += left_width - 1;
                info_offset++;
            }
            info_offset--;
        ...
        insert_colored(left_buffer, left_style_buffer, 1, 4 + info_offset, project.name, left_y == -1 && x == 0 ? "\033[7;91m" : "\033[91m");
        add_tree_to_buffer(left_buffer, left_style_buffer, tree, 1, tree_offset + info_offset,
                           project.path.length(), x == 0 ? left_y : -1,
                           tree_starting_index);

        // RIGHT BUFFER
        info_offset = 0;
        insert_colored(right_buffer, right_style_buffer, 1, 1, "Todo List", x == 1 ? "\033[7;1m" : "\033[1m");
        ...
        if (right_info.size() < max_info_width) {
            insert_colored(right_buffer, right_style_buffer, 1, 2, right_info, "\033[3;36m");
        } else {
            size_t pos = 0;
            while (right_info.size() > pos) {
                std::string info = right_info.substr(pos, max_info_width - 1);
                insert_colored(right_buffer, right_style_buffer, 1, 2 + info_offset, info, "\033[3;36m");
        ...
        for (size_t i = 0;
            ...
            if (i + todo_starting_index == static_cast<size_t>(right_y)) {
                insert_colored(right_buffer, right_style_buffer, 1, 4 + info_offset + i,
        ...
        // GIT STATUS
        insert_colored(right_buffer, right_style_buffer, 1, screen_height - 10, "Git status",
                       "\033[1m");
        ...
        std::vector<styling> style_buffer = combine_buffers(buffer, left_buffer, left_style_buffer, right_buffer, right_style_buffer, left_width);
        add_border(buffer, screen_width);
        ...
        if (x == 0) {
            cursor_x = 1;
            cursor_y = left_y - tree_offset - info_offset;
        } else {
            cursor_x = middle + 1;
            cursor_y = right_y - todo_starting_index - info_offset;
        }
        ch = getchar();
        if (ch == 27) {
            ch = getchar();
            ...
            if (x == 0) {  // left buffer submit
                ...
                if (choice_popup(buffer, todos[right_y].task, "Delete", "Cancel", screen_width) == 0) {
                    if (choice_popup(buffer, "Are you sure?", "Yes", "No", screen_width) == 0) { 
...
void popup(std::vector<std::string> &buffer, const std::string message, int screen_width, bool has_ok) {
    int width = 32;
    int height = 7;
    ...
    insert_colored(center_buffer, style_buffer, message_pos, 2, message, "\033[1;38;5;99m");
    if (has_ok) {
        insert_colored(center_buffer, style_buffer, middle - 1, 4, "Ok", "\033[7;3m");
    }
...
int choice_popup(std::vector<std::string> &buffer, const std::string message,
              const std::string left, const std::string right, int screen_width) {
    int width = 48;
    int height = 9;
    std::vector<std::string> center_buffer(height, std::string(width, ' '));
    std::vector<styling> style_buffer;
    ...
    while (true) {
        ...
        if (size < max_width_task) {
            ...
            while (size > pos) {
                ...
                insert_colored(center_buffer, style_buffer, message_pos, 2 + task_offset, info, "\033[1;38;5;99m");
                pos += max_width_task;
                task_offset++;
            }
            task_offset--;
        ...
        insert_colored(center_buffer, style_buffer, left_pos, 6, left, choice == 0 ? "\033[7;3m" : "");
        insert_colored(center_buffer, style_buffer, right_pos, 6, right, choice == 1 ? "\033[7;3m" : "");
...
std::string input_popup(std::vector<std::string> &buffer,
    ...
    while (true) {
        ...
        insert_colored(center_buffer, style_buffer, message_pos, 2, message, "\033[1;38;5;99m");
        insert_colored(center_buffer, style_buffer, info_pos, 3, info, "\033[3;36m");
